...
class ResPartner(models.Model):
    ...
    cuit = fields.Char(compute='_compute_formated_cuit', )

    @api.depends('vat')
    def _compute_formated_cuit(self):
        for rec in self:
           if rec.vat and rec.afip_tipo_documento == '80':
              rec.cuit = rec.vat.replace('-', '')
    ...
    def get_key_and_certificate(self, environment_type):
        ...
        * en registros de esta misma clase
        ...
        # not certificate on bd, we search on odo conf file
            ...
            msg = _('Not confirmed certificate for %s on company %s') % (environment_type, self.name)
            pkey_path = False
            cert_path = False
